    ...
    public static ArrayList<Data> extractEarthquakes() {
        ...
        try {
            ...
            for(int i=0;i<features.length();i++)
            {
                ...
                String s1=timeFormat.format(date);
                String s=dateFormat.format(date);
                Double mag=properties.getDouble("mag");
                DecimalFormat decimalFormat=new DecimalFormat("0.0");
                String s2=decimalFormat.format(mag);
                Data data=new Data(Double.parseDouble(s2),properties.getString("place"),s1,s,properties.getString("url"));
